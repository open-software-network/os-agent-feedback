...
## Define product context fields

By default a product shares Epode's built-in global field catalog. A product can instead define its own closed
catalog; as soon as one enabled definition exists, it replaces the global catalog for that product's requests.
Every request stores an immutable snapshot of the fields it may collect, so editing or deleting a definition
...
- `type` is exactly `intent`, `preference`, or `constraint`.
