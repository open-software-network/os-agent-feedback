...
Put the outcome action near the start of the MCP server's `instructions`, and keep the full report shape in the
native `report_product_feedback` input schema. The result-local `_agentFeedback` block supplies the scoped
handle and timing; it is not a substitute for registering the tool.
