...
| --- | --- | --- | --- |
| Search or retrieval API | Selected result responses | Each agent-requested result | Server-issued product journey, if available |
| Async crawl or export | Terminal result | Completed or failed outcome the agent can evaluate | Authorized product job ID |
| Browser automation MCP | All browser tools | Close/finalize, or another real journey boundary | Server-owned browser journey ID |
| Documentation MCP | Resolve and query tools | Answer-bearing query | Server-owned research journey, if any |
| Email, payments, or issue MCP | Selected side-effecting tools | Each completed business operation | Authorized product workflow ID |
...
| Serverless API | The same outcome boundary as a long-lived server | Each finite result | Product-owned journey ID; flush telemetry with `waitUntil` |
| Static site or hosted docs | Finite HTML through a trusted reverse proxy | Agent-requested documentation page | None unless verified edge authentication supplies one |
...
When your team controls the public edge route, use the [static docs trusted-edge proxy](/integrations/static-edge).
It preserves the upstream body and ordinary public cache policy, keeps the product key in an edge secret, and
sends feedback reports directly to Epode. Bind it only to dedicated public docs routes, never a hostname-wide
catch-all; `include` provides a second fail-closed path boundary. If you cannot control the public response route,
...
Use `includeTools` for the complete trace and `feedbackTools` for the final boundary. A `sessionRef` extractor
...
Derive `accountRef` and `userRef` from verified API or OAuth context and `anonymousRef` from a product-owned
...
Extract `accountRef` from verified server-side auth after the tenant has been selected. Use a stable opaque
