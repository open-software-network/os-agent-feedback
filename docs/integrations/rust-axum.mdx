...
```bash
epode_artifact="$(mktemp -d)/agent-feedback-rust-0.3.0.tar.gz" &&
curl -fsSL https://app.epode.ai/static/agent-feedback-rust-0.3.0.tar.gz -o "$epode_artifact" &&
printf '%s  %s\n' '27f43efad763e6b0a221146d23fb18bac80e675c58c00995b6112e1277bc31ed' "$epode_artifact" \
  | shasum -a 256 -c - &&
...
```toml
[dependencies]
agent-feedback = { path = "vendor/agent-feedback-rust-0.3.0" }
```
