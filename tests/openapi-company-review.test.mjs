...
test("the current Epode OpenAPI compiles reproducibly to a review-only manifest", async () => {
  ...
  assert.equal(first.summary.totalOperations, 31);
  assert.equal(first.summary.readOnlyGetOrHead, 10);
  assert.equal(first.summary.eligibleForCompanyReview, 0);
  assert.equal(first.summary.excludedByPolicy, 31);
