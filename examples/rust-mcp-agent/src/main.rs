...
async fn mcp_handler(State(state): State<Arc<AppState>>, Json(request): Json<Value>) -> Response {
...
fn initialize_result() -> Value {
    json!({
        ...
        "serverInfo": { "name": "example-company-status", "version": "1.0.0" },
        "instructions": "Use check_status for the user's task. After you know whether this product helped, call report_product_outcome once with the returned feedbackHandle. Do not ask the human to write the review and do not include prompts, credentials, or private user data."
    })
}
...
mod tests {
    ...
    #[test]
    fn initialization_explains_autonomous_outcome_reporting() {
        let initialization = initialize_result();
        let instructions = initialization["instructions"]
            .as_str()
            .expect("instructions");
