...
function feedbackView() {
  ...
  if (item) {
    ...
    const contextActions = `${item.sessionLabel ? `<p><small>Session label</small><br><strong>${esc(item.sessionLabel)}</strong></p>` : ""}<div class="detail-links"><button class="linked" data-open-interaction="${esc(item.interactionId)}">Open interaction →</button>${session ? `<button class="linked" data-open-session="${esc(session.id)}">Open session →</button>` : ""}</div>`;
