...
function breakdown(rows, total, fallback, { raw = false } = {}) {
  if (!rows.length) return `<p class="muted">${esc(fallback)}</p>`;
  const max = Math.max(...rows.map((row) => row.count), 1);
  return `<div class="breakdown">${rows.map((row) => `<div><span><b>${esc(raw ? row.name : title(row.name))}</b><small>${row.count} · ${total ? Math.round(row.count / total * 100) : 0}%</small></span><i><em data-bar-width="${Math.round(row.count / max * 100)}"></em></i></div>`).join("")}</div>`;
}
...
function feedbackView() {
  ...
  if (item) {
    ...
    const contextActions = `<div class="detail-links"><button class="linked" data-open-interaction="${esc(item.interactionId)}">Open interaction →</button>${session ? `<button class="linked" data-open-session="${esc(session.id)}">Open journey →</button>` : ""}</div>`;
...
function homeView() {
  ...
  const context = `<p class="page-context">A current view of agent experience journeys, what agents reported, and where your team should look next.</p>`;
  ...
  const lostDemand = dashboard.insights.lostDemand || { decisionInteractions: 0, zeroMatchDecisions: 0, expressedDimensions: [], violatedDimensions: [], counterfactualChanges: [], medianCounterfactualDelta: null };
  const journeyFlow = dashboard.insights.journeyFlow || { edges: [], exitOperations: [] };
  const handoff = dashboard.insights.handoff || { handoffClicks: 0, sessionsWithHandoff: 0, sessions: 0, handoffRate: 0, landingOperations: [] };
  const signalOutcomes = dashboard.insights.signalOutcomes || [];
  const agentVendors = dashboard.insights.agentVendors || [];
  const rankPositions = dashboard.insights.rankPositions || [];
  const unknownDimensions = dashboard.insights.unknownDimensions || [];
  const unansweredQuestions = dashboard.insights.unansweredQuestions || [];
  const journeyFunnel = dashboard.insights.journeyFunnel || { arrived: 0, enteredGraph: 0, expressedNeeds: 0, reachedDecision: 0, handoffFollowed: 0, tokenedFetchRate: 0 };
  const trafficClasses = dashboard.insights.trafficClasses || [];
  const channels = dashboard.insights.channels || [];
  const offGraphAttempts = dashboard.insights.offGraphAttempts || { attempts: 0, operations: [] };
  const journeyRealitySection = `<section class="funnel"><div><p class="eyebrow">JOURNEY REALITY</p><h2>How agents actually shop</h2></div><div class="funnel-steps"><button data-investigate-view="sessions"><strong>${journeyFunnel.arrived}</strong><span>Arrived</span></button><i>→</i><button data-investigate-view="sessions"><strong>${journeyFunnel.enteredGraph}</strong><span>Entered the graph</span></button><i>→</i><button data-investigate-view="sessions"><strong>${journeyFunnel.expressedNeeds}</strong><span>Expressed needs</span></button><i>→</i><button data-investigate-view="sessions"><strong>${journeyFunnel.reachedDecision}</strong><span>Reached a decision</span></button><i>→</i><button data-investigate-view="sessions"><strong>${journeyFunnel.handoffFollowed}</strong><span>Handoff followed</span></button></div><p class="comparison-note">Tokened-fetch rate ${journeyFunnel.tokenedFetchRate}%: of agent sessions that arrived, how many made a tokened graph fetch. An agent that arrives but never enters silently defects to a competitor.</p></section>`;
  const channelSection = `<div class="home-grid"><section class="home-panel"><div class="section-heading"><div><p class="eyebrow">CHANNELS</p><h2>Faceted links vs native graph</h2></div></div>${breakdown(channels, channels.reduce((sum, row) => sum + row.count, 0), "Channels appear when graph hops carry a channel-tagged experience payload.", { raw: true })}</section><section class="home-panel"><div class="section-heading"><div><p class="eyebrow">OFF-GRAPH ATTEMPTS</p><h2>Where agents fell off the graph</h2></div><span>${offGraphAttempts.attempts} attempt${offGraphAttempts.attempts === 1 ? "" : "s"}</span></div>${breakdown(offGraphAttempts.operations, offGraphAttempts.attempts, "No fabricated URLs or malformed graph fetches from agent traffic.", { raw: true })}</section></div>`;
  const zeroMatchShare = lostDemand.decisionInteractions ? `${Math.round(lostDemand.zeroMatchDecisions / lostDemand.decisionInteractions * 100)}%` : "—";
  const medianGap = lostDemand.medianCounterfactualDelta == null ? "—" : Math.round(lostDemand.medianCounterfactualDelta * 100) / 100;
  const lostDemandSection = `<section class="funnel"><div><p class="eyebrow">LOST DEMAND</p><h2>What agents asked for and could not get</h2></div><div class="funnel-steps"><button data-investigate-view="interactions"><strong>${lostDemand.decisionInteractions}</strong><span>Decisions evaluated</span></button><i>→</i><button data-investigate-view="interactions"><strong>${lostDemand.zeroMatchDecisions}</strong><span>Zero exact matches (${zeroMatchShare})</span></button><i>→</i><button data-investigate-view="interactions"><strong>${esc(medianGap)}</strong><span>Median gap to a match</span></button></div></section><div class="three-col breakdown-grid"><article><h2>Stated demand</h2>${breakdown(lostDemand.expressedDimensions, lostDemand.expressedDimensions.reduce((sum, row) => sum + row.count, 0), "No expressed dimensions yet.", { raw: true })}</article><article><h2>Dealbreaker dimensions</h2>${breakdown(lostDemand.violatedDimensions, lostDemand.violatedDimensions.reduce((sum, row) => sum + row.count, 0), "No hard-constraint violations yet.", { raw: true })}</article><article><h2>Cheapest fixes</h2>${breakdown(lostDemand.counterfactualChanges, lostDemand.counterfactualChanges.reduce((sum, row) => sum + row.count, 0), "No counterfactuals yet. They appear when a search ends with zero exact matches.", { raw: true })}</article></div>`;
  const edgeRows = journeyFlow.edges.map((edge) => ({ name: `${edge.fromOperation} → ${edge.toOperation}`, count: edge.traversals }));
  const journeyFlowSection = `<div class="home-grid"><section class="home-panel"><div class="section-heading"><div><p class="eyebrow">JOURNEY FLOW</p><h2>Where agents go next</h2></div><button class="link-button" data-view="sessions">View journeys</button></div>${breakdown(edgeRows, edgeRows.reduce((sum, row) => sum + row.count, 0), "Transitions appear when journeys carry a session reference.", { raw: true })}</section><section class="home-panel"><div class="section-heading"><div><p class="eyebrow">DROP-OFF</p><h2>Where journeys end</h2></div><button class="link-button" data-view="sessions">View journeys</button></div>${breakdown(journeyFlow.exitOperations, journeyFlow.exitOperations.reduce((sum, row) => sum + row.count, 0), "No journey exits in the last 30 days.", { raw: true })}</section></div>`;
  const handoffSection = `<section class="funnel"><div><p class="eyebrow">HANDOFF</p><h2>From agent journey to human click</h2></div><div class="funnel-steps"><button data-investigate-view="sessions"><strong>${handoff.sessions}</strong><span>Proven sessions</span></button><i>→</i><button data-investigate-view="sessions"><strong>${handoff.sessionsWithHandoff}</strong><span>Sessions with a handoff (${handoff.handoffRate}%)</span></button><i>→</i><button data-investigate-view="interactions"><strong>${handoff.handoffClicks}</strong><span>Product link clicks</span></button></div></section>${handoff.landingOperations.length ? `<div class="three-col breakdown-grid"><article><h2>Handoff landing pages</h2>${breakdown(handoff.landingOperations, handoff.handoffClicks, "", { raw: true })}</article></div>` : ""}`;
  const signalMax = Math.max(...signalOutcomes.map((row) => row.conversions), 1);
  const signalRows = signalOutcomes.map((row) => `<div><span><b>${esc(row.signal)}</b><small>${row.decisions} decision${row.decisions === 1 ? "" : "s"} · ${row.outcomes} outcome${row.outcomes === 1 ? "" : "s"} · ${row.conversions} conversion${row.conversions === 1 ? "" : "s"}</small></span><i><em data-bar-width="${Math.round(row.conversions / signalMax * 100)}"></em></i></div>`).join("");
  const vendorLabels = { claude: "Claude", openai: "OpenAI", perplexity: "Perplexity", gemini: "Gemini", cohere: "Cohere", copilot: "Copilot", other: "Other" };
  const vendorRows = agentVendors.map((row) => ({ name: vendorLabels[row.vendor] || row.vendor, count: row.interactions }));
  const classLabels = { declared_agent: "Declared agent", suspected_cloud_agent: "Suspected cloud agent", human: "Human" };
  const classMax = Math.max(...trafficClasses.map((row) => row.sessions), 1);
  const classRows = trafficClasses.map((row) => `<div><span><b>${esc(classLabels[row.class] || row.class)}</b><small>${row.sessions} session${row.sessions === 1 ? "" : "s"} · ${row.interactions} interaction${row.interactions === 1 ? "" : "s"}</small></span><i><em data-bar-width="${Math.round(row.sessions / classMax * 100)}"></em></i></div>`).join("");
  const contextSection = `<div class="home-grid"><section class="home-panel"><div class="section-heading"><div><p class="eyebrow">CONTEXT ROI</p><h2>Signals that drive outcomes</h2></div></div>${signalRows ? `<div class="breakdown">${signalRows}</div>` : `<p class="muted">No personalization decisions cited customer signals yet.</p>`}</section><section class="home-panel"><div class="section-heading"><div><p class="eyebrow">AGENT MIX</p><h2>Traffic by class and assistant runtime</h2></div></div>${classRows ? `<div class="breakdown">${classRows}</div>` : `<p class="muted">Traffic classes appear when proven sessions are recorded.</p>`}<h3>Within declared agent traffic</h3>${breakdown(vendorRows, vendorRows.reduce((sum, row) => sum + row.count, 0), "Runtime evidence has not named an assistant yet.", { raw: true })}<p class="privacy-note"><b>Unverified evidence.</b> Runtime hints and user-agent values are observations, never identity. A suspected cloud agent is a behavioral upper bound — a human opening an agent-composed link looks the same at its first hop.</p></section></div>`;
  const rankRows = rankPositions.map((row) => ({ name: `Position ${row.name}`, count: row.count }));
  const unknownsSection = `<div class="three-col breakdown-grid"><article><h2>Result position views</h2>${breakdown(rankRows, rankRows.reduce((sum, row) => sum + row.count, 0), "No search-attributed item views yet.")}</article><article><h2>Unknown dimensions</h2>${breakdown(unknownDimensions, unknownDimensions.reduce((sum, row) => sum + row.count, 0), "Agents answered every dimension they were asked.", { raw: true })}</article><article><h2>Unanswered questions</h2>${breakdown(unansweredQuestions, unansweredQuestions.reduce((sum, row) => sum + row.count, 0), "No declined or unanswerable context questions.", { raw: true })}</article></div>`;
  return `${header("HOME", dashboard.currentProduct.name, "Last 30 days", actions)}${context}${metricStrip([[dashboard.insights.opportunities, "Opportunities"], [confirmed, "Confirmed"], [confirmed ? `${dashboard.insights.reviewRate}%` : "—", "Feedback rate"], [blockers, "Reports with blockers", blockers ? "negative" : ""], [workarounds, "Workarounds used"]])}<p class="comparison-note">Recent ${dashboard.insights.comparisonDays}-day activity: ${dashboard.insights.recentOpportunities} opportunities · ${dashboard.insights.recentConfirmedInteractions} confirmed · ${dashboard.insights.recentReports} reports. Previous ${dashboard.insights.comparisonDays} days: ${dashboard.insights.previousOpportunities} · ${dashboard.insights.previousConfirmedInteractions} · ${dashboard.insights.previousReports}.</p>${investigations}${activity}<section class="funnel"><div><p class="eyebrow">CONVERSION</p><h2>From product response to feedback</h2></div><div class="funnel-steps"><button data-investigate-view="interactions"><strong>${dashboard.insights.opportunities}</strong><span>Opportunities</span></button><i>→</i><button data-investigate-view="interactions" data-investigate-filter="confirmed"><strong>${confirmed}</strong><span>Confirmed</span></button><i>→</i><button data-investigate-view="feedback"><strong>${dashboard.insights.reports}</strong><span>Reported</span></button></div></section>${journeyRealitySection}${lostDemandSection}${journeyFlowSection}${handoffSection}${contextSection}${channelSection}${unknownsSection}<div class="three-col breakdown-grid"><article><h2>Impacts</h2>${breakdown(impactRows, dashboard.insights.reports, "No impact data yet.")}</article><article><h2>Finding types</h2>${breakdown(findingRows, findingRows.reduce((sum, row) => sum + row.count, 0), "No findings yet.")}</article><article><h2>Topics</h2>${breakdown(topicRows, topicRows.reduce((sum, row) => sum + row.count, 0), "No topics yet.")}</article></div><section class="explanation compact"><h2>How Epode counts activity</h2><p>HTTP responses begin as opportunities—not assumed agent traffic. A submitted report confirms the interaction. MCP tool calls are confirmed immediately because the protocol proves a tool-capable client used them.</p><p>Latency is reported as p50 ${duration(dashboard.insights.p50DurationMs)} and p95 ${duration(dashboard.insights.p95DurationMs)} across responses that supplied timing.</p></section>`;
}
...
function interactionsView() {
  ...
  if (interaction) {
    ...
    return `${header("INTERACTION", interaction.operation, date(interaction.occurredAt), detailActions())}<button class="back" data-back="interactions">← All interactions</button>${metricStrip([[title(interaction.classification), "Classification", interaction.classification === "confirmed" ? "positive" : "neutral"], [title(interaction.surface), "Surface"], [interaction.statusCode || "—", "HTTP status"], [duration(interaction.durationMs), "Duration"]])}<div class="detail-layout"><div class="detail-main"><section class="detail-section"><div class="section-heading"><div><p class="eyebrow">EVIDENCE</p><h2>What proves this interaction</h2></div>${badge(interaction.classification)}</div><p>${interaction.classification === "confirmed" ? `Confirmed by ${esc(title(interaction.confirmationMethod || "feedback report"))}.` : "This response carried feedback instructions, but no agent action has confirmed it yet."}</p></section><section class="detail-section"><div class="section-heading"><h2>Agent feedback</h2>${linked ? badge(reportImpact(linked)) : ""}</div>${linked ? `<p class="quote">“${esc(linked.summary)}”</p><small>${reportFindings(linked).length} finding${reportFindings(linked).length === 1 ? "" : "s"}</small><button class="linked" data-open-feedback="${esc(linked.id)}">Open feedback →</button>` : `<div class="inline-empty"><p>No feedback was submitted for this interaction.</p><small>This is expected for many HTTP opportunities.</small></div>`}</section>${session ? `<section class="detail-section"><h2>Session context</h2><p>This interaction belongs to a continuity group supplied by ${esc(title(session.source))}.</p><button class="linked" data-open-session="${esc(session.id)}">Open journey →</button></section>` : ""}</div><aside class="detail-sidebar"><section><h2>Properties</h2>${propertyList([["Occurred", esc(date(interaction.occurredAt))], ["Operation", esc(interaction.operation)], ["Customer", esc(interaction.customerRef || interaction.customerId || "Not linked")], ["Confirmation", esc(title(interaction.confirmationMethod || "none"))], ["Runtime hint", esc(interaction.runtimeHint || "Not provided")], ["Hint provenance", esc(interaction.runtimeHintSource || "Not provided")]])}</section><p class="privacy-note"><b>No agent identity.</b> Epode stores only evidence supplied by the product or protocol.</p><code class="object-id">${esc(interaction.id)}</code></aside></div>`;
  }
...
function sessionsView() {
  ...
  if (session) {
    ...
    const sessionCustomer = session.customerDisplayName || session.customerRef || session.customerId;
    const customers = sessionCustomer ? [sessionCustomer] : [...new Set(interactions.map((entry) => entry.customerRef || entry.customerId).filter(Boolean))];
    ...
    return `${header("JOURNEY", `Journey ${session.refHint}…`, `${interactions.length} interactions`, detailActions())}<button class="back" data-back="sessions">← All journeys</button>${metricStrip([[interactions.length, "Interactions"], [confirmed, "Confirmed"], [reviewed.length, "Reviews"], [sessionDuration(session), "Duration"]])}<div class="detail-layout session-detail"><div class="detail-main"><section class="detail-section"><div class="section-heading"><div><p class="eyebrow">TIMELINE</p><h2>Interaction journey</h2></div><span>${esc(date(session.startedAt))}</span></div>${timeline ? `<div class="session-timeline">${timeline}</div>` : `<div class="inline-empty"><p>No interactions are currently loaded for this session.</p></div>`}</section></div><aside class="detail-sidebar"><section><h2>Journey properties</h2>${propertyList([["Proof source", esc(title(session.source))], ["Started", esc(date(session.startedAt))], ["Last seen", esc(date(session.lastSeenAt))], ["Customer", esc(customers.length === 1 ? customers[0] : customers.length > 1 ? "Multiple customer refs" : "Not linked")]])}</section><p class="privacy-note"><b>Proof-based continuity.</b> This journey exists only because an experience graph or product/MCP protocol supplied a stable reference. Epode never groups by time or inferred identity.</p><code class="object-id">${esc(session.id)}</code></aside></div>`;
  }
  if (!dashboard.sessions.length) return `${header("JOURNEYS", "Agent experience", "0 journeys", `<button class="button" data-refresh-data>Refresh</button>`)}<p class="page-context">Journeys connect interactions when an experience graph or product session reference supplies continuity. Epode never guesses continuity from timing or identity.</p>${empty("No proven journeys", "This is normal. Feedback and interactions work without a journey.", "interactions")}`;
  ...
  const rows = filtered.map((entry) => {
    ...
    const sessionCustomer = entry.customerDisplayName || entry.customerRef || entry.customerId;
    const customers = sessionCustomer ? [sessionCustomer] : [...new Set(interactions.map((interaction) => interaction.customerRef || interaction.customerId).filter(Boolean))];
    return `<button class="table-row session-columns" data-session="${esc(entry.id)}" aria-label="Open journey ${esc(entry.refHint)}. Source ${esc(title(entry.source))}. ${interactions.length} interactions and ${reports.length} feedback reports. Duration ${esc(sessionDuration(entry))}. Last seen ${esc(date(entry.lastSeenAt))}."><span class="primary-cell"><strong>${esc(entry.refHint)}…</strong><small>${esc(title(entry.source))}</small></span><span>${interactions.length}</span><span>${sessionFeedbackSummary(reports)}</span><span>${esc(customers.length === 1 ? customers[0] : customers.length > 1 ? "Multiple" : "Not linked")}</span><span>${sessionDuration(entry)}</span><time title="${esc(date(entry.lastSeenAt))}">${esc(relativeDate(entry.lastSeenAt))}</time></button>`;
  }).join("");
  const toolbar = explorerToolbar({ placeholder: "Search journey, operation, or source", primaryLabel: "Feedback", primaryOptions: [["all", "All journeys"], ["reviewed", "Has feedback"], ["unreviewed", "No feedback"]], secondaryLabel: "Proof source", secondaryOptions: sources });
  const table = rows ? `<div class="explorer-table"><div class="table-head session-columns"><span>Journey</span><span>Interactions</span><span>Feedback</span><span>Customer</span><span>Duration</span><span>Last seen</span></div>${rows}</div>` : `<div class="filtered-empty"><h2>No matching journeys</h2><p>Journeys only exist when an experience graph or product supplies proof of continuity.</p><button class="button" data-clear-filters>Clear filters</button></div>`;
  const total = dashboard.listState?.sessionsTotal ?? dashboard.sessions.length;
  return `${header("JOURNEYS", "Agent experience", `${filtered.length} matching · ${total} retained`, `<button class="button" data-refresh-data>Refresh</button>`)}<p class="page-context">Journeys connect interactions when an experience graph or product session reference supplies continuity. Epode never guesses continuity from timing or identity.</p>${metricStrip([[filtered.length, "Loaded matches"], [totalInteractions, "Loaded interactions"], [reviewedSessions, "With feedback"], [filtered.length ? (totalInteractions / filtered.length).toFixed(1) : "—", "Interactions per journey"]])}${toolbar}${table}${listContinuation("sessions", dashboard.sessions.length, total)}`;
}
...
const linkedSessionContract = "Use a stable typed Customer identity and a stable opaque workflow ID owned by your product. Use sessionRef only from server/product state: validate authenticated ownership continuity before passing it. Reuse the canonical ID returned by workflow creation for follow-ups, cached results, and deduplicated creates; omit sessionRef when proof is missing or invalid. Never substitute request/trace IDs, telemetry-only cache keys, transport sessions, caller-controlled or model/tool-proposed values, timestamps, prompts, labels, or customerRef. With typed refs, customerId may show resolved linkage while raw customerRef stays absent; if legacy Ask-once customerRef is configured, it is retained and may be displayed but is never Session proof. Never persist or display plaintext account/user/anonymous/session refs, arguments, prompts, results, credentials, or exceptions.";
const linkedSessionVerification = "For the same Customer: create Run A, replay its cached/deduplicated result, and send an ordered follow-up using its canonical create-result ID; verify all are in Session A. Create Run B with its own ordered follow-up and verify a separate Session B with no mixing. In response records, verify missing/invalid ownership proof stays unlinked. In the product client, verify normal calls survive an Epode outage. Audit persistence/dashboard for plaintext typed identity/session refs, arguments, prompts, results, credentials, or exceptions.";
...
function setupInstructions() {
  ...
  const nodeDownload = `mkdir -p .epode/artifacts &&\nepode_artifact=".epode/artifacts/agent-feedback-node-0.4.0.tgz" &&\ncurl -fsSL ${artifacts}/agent-feedback-node-0.4.0.tgz -o "$epode_artifact" &&\nprintf '%s  %s\\n' 'ecad0dc8b17c5d973e1f97b4f691b7076af223aaa1a9ac697daa499b59c585e3' "$epode_artifact" | shasum -a 256 -c -`;
  const pythonDownload = verifiedDownload("agent_feedback-0.4.0-py3-none-any.whl", "a3d79b6d5eb5fe101f2cd7a6c1ca4ae2415358b81276fde06d30bbf3aef2b685");
  const goDownload = verifiedDownload("agent-feedback-go-0.4.0.tar.gz", "5d4ee89ad47d61a7d3f38dbd6320585b4e1c4de40beae12d526c3858683ad85e");
  const rustDownload = verifiedDownload("agent-feedback-rust-0.4.0.tar.gz", "cd6eb2361095a37efeebcc92ba029bfca696cab67e0b0815d90b1432f104bd9e");
  const protocolDownload = verifiedDownload("agent-feedback-protocol-v1-0.4.0.zip", "ae3a670fcaf903c781155ec5002874cbe079996b2c6d28221414100e5d86735b");
  ...
  const instructions = {
    ...
    "manual-mcp": {
      ...
      code: `1. Implement stateless MCP 2026-07-28 and server/discover.\n2. Authenticate and authorize the tenant before forwarding /mcp. Emit accountRef/userRef only from verified product identity and anonymousRef only from a product-owned first-party visitor ID.\n3. Keep customerRef only as the durable Ask once subject. Treat typed tool input/result IDs as candidates only; sessionRef is the owned canonical workflow ID returned or stored by create/cache/dedup after product-registry ownership validation. Cached or deduplicated creates return and reuse the original canonical ID.\n4. Validate MCP-Protocol-Version, Mcp-Method, and Mcp-Name.\n5. Emit confirmed telemetry and add _agentFeedback to selected product-tool results.\n6. Register record_product_feedback_consent and report_product_feedback with strict schemas.\n7. From consent_required allow only the consent tool; from feedback_ready allow only the report tool.`,
    ...
    "python-asgi": {
      ...
      code: `from agent_feedback import AgentFeedbackASGI\n\napp = AgentFeedbackASGI(\n    app,\n    api_key=os.environ["AGENT_FEEDBACK_KEY"],\n    include=("${route}",),\n    account_ref=lambda scope: scope.get("state", {}).get("account_id"),\n    user_ref=lambda scope: scope.get("state", {}).get("user_id"),\n    anonymous_ref=lambda scope: scope.get("state", {}).get("first_party_visitor_id"),\n    customer_ref=lambda scope: scope.get("state", {}).get("account_id"), # durable Ask once compatibility\n    session_ref=lambda scope: scope.get("state", {}).get("owned_canonical_workflow_id"), # owned canonical workflow ID returned/stored by create/cache/dedup after ownership validation\n)\n# Authentication is the outer wrapper and populates verified/product-owned state first.\napp = ProductAuthenticationASGI(app)`,
      advanced: `Omit any identity or journey reference your product cannot prove.`,
    ...
    "static-edge": {
      ...
      install: nodeInstall,
      code: `import { createStaticDocsProxy } from "@epode/node/edge";
...
export default {
  fetch(request, env, context) {
    ...
    "manual-http": {
      ...
      install: protocolDownload,
      code: `GET ${location.origin}/.well-known/agent-feedback-v1.json\n\n1. Emit accountRef/userRef only from verified product authentication and anonymousRef only from a product-owned first-party visitor ID. Keep customerRef only for durable Ask once.\n2. Use sessionRef only for the owned canonical workflow ID returned/stored by create/cache/dedup after ownership validation; cached or deduplicated creates return and reuse the original canonical ID. Never trust caller or agent arguments for identity or grouping.\n3. Derive an opaque consent subject from your product key and customerRef.\n4. Resolve Ask once state through /api/v2/consent/state.\n5. Unknown: emit the answer-first decision action. Approved: emit the report contract. Declined: emit neither.\n6. Queue opportunity telemetry without failing the product response.`,
...
function setupAgentPrompt(integration) {
  ...
  const identityRequirement = setupSurface === "static" ? "- Public documentation needs no customer identity. Add accountRef/userRef only from verified edge authentication and anonymousRef only from a product-owned first-party visitor ID. Keep customerRef only for durable Ask once and exactly equal to accountRef; never pair it with userRef alone." : "- Run product authentication and authorized tenant selection before Agent Feedback. accountRef and userRef come only from verified product identity; anonymousRef comes only from a product-owned first-party pre-login ID. Keep customerRef only for durable Ask once compatibility and exactly equal to accountRef; never pair it with userRef alone. Never use names, emails, caller input, prompts, or agent/tool arguments.";
...
function productCreateView(firstProduct = false) {
  ...
  const copy = firstProduct ? "Products keep journeys, feedback, and interactions separate. Start with the product your customers' agents use." : "The new product gets its own Home overview, experience graph integration, journeys, feedback, and sessions.";
...
function setupView() {
  ...
  const identityCopy = setupSurface === "static" ? "Public documentation needs no customer identity. Add verified account or user references only when trusted edge authentication provides them; a product-owned first-party visitor ID may remain anonymous. Durable Ask once customerRef must exactly equal accountRef." : "Use <b>accountRef</b> and <b>userRef</b> only for identity your product verified, and <b>anonymousRef</b> only for a product-owned first-party visitor. Keep <b>customerRef</b> only when Ask once must remember permission, exactly equal to accountRef and never paired with userRef alone. Never use names, emails, caller values, prompts, or agent arguments.";
  const manualInstall = `<div class="install-panel"><h3>Install</h3><div class="copy-block"><pre><code>${esc(integration.install)}</code></pre><button class="button" data-copy="${esc(integration.install)}">Copy</button></div><h3>Configure once</h3><p>${identityCopy}</p>${setupSurface === "static" ? "" : `<p>${esc(linkedSessionContract)}</p>`}<div class="copy-block"><pre><code>${esc(integration.code)}</code></pre><button class="button" data-copy="${esc(integration.code)}">Copy</button></div>${integration.advanced ? `<details><summary>Optional session grouping</summary><p>Only supply a session reference when your product already has proof that interactions belong together.</p><pre><code>${esc(integration.advanced)}</code></pre></details>` : ""}</div>`;
  ...
  const linkedSessionChecklist = setupSurface === "static" ? "" : `<section class="setup-step"><div class="step-number">○</div><div class="step-body"><p class="eyebrow">MANUAL CHECK · NOT YET VERIFIED</p><h2>Verify workflow continuity</h2><p>${esc(linkedSessionVerification)}</p><small>The interaction and report statuses above remain valid transport milestones; they do not prove exact correlation or complete this manual check.</small></div></section>`;
...
function render() {
  ...
  page.querySelectorAll?.("[data-bar-width]").forEach((el) => {
    el.style.width = `${Math.min(100, Math.max(0, Number(el.dataset.barWidth) || 0))}%`;
  });
