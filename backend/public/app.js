...
function header(kicker, heading, meta = "", extra = "") {
  return `<header><div class="page-heading"><p class="eyebrow">${esc(kicker)}</p><h1>${esc(heading)}</h1></div><div class="header-actions"><span class="page-meta">${esc(meta)}</span>${extra}</div></header>`;
}

function empty(heading, copy, view = "setup") {
  return `<div class="empty"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 12h-6l-2 3h-4l-2-3H2"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg><h2>${esc(heading)}</h2><p>${esc(copy)}</p><button class="button primary" data-view="${esc(view)}">Go to ${esc(view)}</button></div>`;
}
...
function explorerToolbar({ placeholder, primaryLabel, primaryOptions, secondaryLabel = "Surface", secondaryOptions = [] }) {
  ...
  return `<div class="explorer-toolbar" role="search"><label class="explorer-search"><span>Search</span><span class="search-field"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg><input id="explorer-search" type="search" value="${esc(explorerQuery)}" placeholder="${esc(placeholder)}" autocomplete="off"></span></label><label><span>${esc(primaryLabel)}</span><select id="explorer-primary">${options(primaryOptions)}</select></label>${secondary}<label><span>Time range</span><select id="explorer-range"><option value="24h" ${explorerRange === "24h" ? "selected" : ""}>Last 24 hours</option><option value="7d" ${explorerRange === "7d" ? "selected" : ""}>Last 7 days</option><option value="30d" ${explorerRange === "30d" ? "selected" : ""}>Last 30 days</option><option value="all" ${explorerRange === "all" ? "selected" : ""}>All loaded data</option></select></label>${explorerQuery || explorerPrimary !== "all" || explorerSecondary !== "all" || explorerRange !== "30d" ? `<button class="button sm filter-reset" data-clear-filters>Reset</button>` : ""}</div>`;
}

function detailActions() {
  return `<button class="button sm" data-copy-page-link>Copy link</button>`;
}
...
function renderLoadError(error) {
  ...
  page.innerHTML = `<div class="empty load-error"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg><h2>${esc(message)}</h2><p>${esc(error?.message || "Check your connection and try again.")}</p><button class="button primary" data-retry-dashboard>Try again</button></div>`;
}
...
function feedbackView() {
  ...
  if (!dashboard.outcomes.length) return `${header("FEEDBACK", "Agent feedback", "0 reviews", `<button class="button sm" data-refresh-data>Refresh</button>`)}${empty("No feedback yet", "Install Epode on an agent-usable product route, then send one real interaction.")}`;
...
function insightsView() {
  if (!dashboard.interactions.length) return `${header("INSIGHTS", "Product health", "No data", `<button class="button sm" data-refresh-data>Refresh</button>`)}${empty("No product activity yet", "Finish setup and send one real product request or MCP tool call.")}`;
...
function interactionsView() {
  ...
  if (!dashboard.interactions.length) return `${header("INTERACTIONS", "Product activity", "0 interactions", `<button class="button sm" data-refresh-data>Refresh</button>`)}${empty("No interactions yet", "Finish setup and use one configured product route.")}`;
  ...
  const table = rows ? `<div class="explorer-table"><div class="table-head interaction-columns"><span>Evidence</span><span>Operation</span><span>Surface</span><span>Status</span><span>Duration</span><span>Feedback</span><span>Occurred</span></div>${rows}</div>` : `<div class="filtered-empty"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m13.5 8.5-5 5"/><path d="m8.5 8.5 5 5"/><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg><h2>No matching interactions</h2><p>Try a wider time range or clear the filters.</p><button class="button" data-clear-filters>Clear filters</button></div>`;
  return `${header("INTERACTIONS", "Product activity", `${filtered.length} of ${ranged.length} interactions`, `<button class="button sm" data-refresh-data>Refresh</button>`)}${metricStrip([[filtered.length, "Interactions"], [confirmed, "Confirmed", "positive"], [reviewed, "With feedback"], [duration(average), "Average duration"]])}${toolbar}${table}`;
}
...
function setupView() {
  ...
  const createKeyButton = setupKey ? `<button class="button sm" data-revoke-key="${esc(setupKey.id)}">Create new key</button>` : "";
...
function teamView() {
  ...
  const renameAction = canInvite ? `<button class="button sm" data-rename-team>Rename team</button>` : "";
