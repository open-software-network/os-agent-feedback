...
# signoff context.
#
# The path filters mirror the dorny/paths-filter globs in
...
if changed '^(backend/|Makefile$|\.github/workflows/ci\.yml$)'; then
  needs_backend=true
fi

if changed '^(package\.json$|pnpm-lock\.yaml$|pnpm-workspace\.yaml$|biome\.json$|Makefile$|tests/|backend/|sdk/|protocol/|docs/|scripts/hosted-artifact-ledger\.json$|scripts/verify-image-artifact-ledger\.sh$|scripts/verify-public-integration-surface\.sh$|\.github/workflows/ci\.yml$)'; then
  needs_node=true
fi

if changed '^(package\.json$|pnpm-lock\.yaml$|pnpm-workspace\.yaml$|biome\.json$|Makefile$|sdk/node/|tests/customer-context-mcp-e2e\.mjs$|\.github/workflows/ci\.yml$)'; then
  needs_sdk_node=true
fi

if changed '^(sdk/|scripts/sdk-release-readiness\.sh$|package\.json$|pnpm-lock\.yaml$|pnpm-workspace\.yaml$|\.github/workflows/ci\.yml$|\.github/workflows/sdk-release\.yml$)'; then
  needs_sdk_packages=true
fi

if changed '^(examples/|backend/public/agent-feedback-node-[^/]*\.tgz$|sdk/node/|\.dockerignore$|\.github/workflows/build-node-examples\.yml$|\.github/workflows/ci\.yml$)'; then
  needs_examples=true
fi

if changed '^(web/|package\.json$|pnpm-lock\.yaml$|pnpm-workspace\.yaml$|biome\.json$|Makefile$|backend/openapi\.json$|\.github/workflows/ci\.yml$)'; then
  needs_web=true
fi

if changed '^(package\.json$|pnpm-lock\.yaml$|pnpm-workspace\.yaml$|Makefile$|docs/|sdk/|protocol/|backend/public/app\.js$|tests/docs-contract\.test\.mjs$|\.github/workflows/ci\.yml$)'; then
  needs_docs=true
fi
...
if changed '^(scripts/local-ci\.sh$|scripts/signoff-)'; then
  needs_backend=true
  needs_e2e=true
  needs_node=true
  needs_sdk_node=true
...
needs_sdk=false
if [[ "$needs_sdk_node" == "true" || "$needs_sdk_packages" == "true" ]]; then
  needs_sdk=true
fi
